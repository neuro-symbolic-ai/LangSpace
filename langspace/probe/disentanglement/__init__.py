...
class DSRDataset(GenerativeDataset):
    def __init__(self, data, gen_factors):
        ...
        index = 0
        for d in data:
            dsr_tags = [k for k in d[1].strip().split(' ') if k in dic]
            structure = [dic[dsr] for dsr in dsr_tags]
            for factor in self.generative_factors:
                if factor in structure:
                    temp_role = []
                    for i in range(0, len(dsr_tags)):
                        if dic[dsr_tags[i]] == factor:
                            temp_role.append(dsr_tags[i])
...
class DisentanglementProbe(LatentSpaceProbe):
    ...
    def __init__(self, model: LangVAE, data: Iterable[Union[str, Sentence]], sample_size: int,
        ...
        # get annotation
        self.dataset = DSRDataset(data[:sample_size], gen_factors)

        # get latent representation
        latent_list = []
        for sent in tqdm(data[:sample_size]):
            _, _, latent = self.encoding(sent[0].strip())
            latent_list.append(latent)
        representations = np.array(latent_list)
    ...
    def mutual_information_estimation(self, num_bins, sample_number, normalize=False):
        ...
        """get the max and min of each dimension 256"""
    ...
    def report(self) -> DataFrame:
        ...
        for metric in self.metrics:
            if metric == "z-diff":
                self.beta_vae_metric()
            elif metric == "z-min-var":
                self.factor_vae_metric()
            elif metric == "MIG":
                self.mutual_information_gap()
            elif metric in ["Modularity", "Explicitness"]:
                self.modularity_explicitness()
            elif metric in ["Disentanglement", "Completeness", "Informativeness"]:
                self.disentanglement_completeness_informativeness()
            else:
                pass
